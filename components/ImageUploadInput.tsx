...
interface Props {
  name?: string;
  setValue: UseFormSetValue<z.infer<typeof blogSchema>>;
...
const ImageUploadInput = ({
  ...
  const { publicId } = useSelector((state: RootState) => state.imageUpload);
  ...
  const uploadImage = async (file: File) => {
    ...
    if (!UPLOAD_PRESET || !CLOUD_NAME) {
      showErrorToast("Error", "Cloudinary configuration is missing");
      return;
    }
    ...
    if (file.size > maxSize) {
      showErrorToast("Error", "Image size should be less than 10MB");
      return;
    }
    ...
    try {
      ...
      xhr.onload = () => {
        ...
        if (xhr.status >= 200 && xhr.status < 300) {
          try {
            ...
            showErrorToast("Error", "Failed to process the upload response");
      ...
      showErrorToast("Error", `Failed to upload image: ${errorMessage}`);
      setError(errorMessage);
      setUploadProgress(null);
    }
  };
